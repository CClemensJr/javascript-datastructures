...
const toTitleCase = (arr) => arr.map(str => str.replace(str[0], str.charAt(0).toUpperCase()));
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(character => {
    if (parseInt(character.mass) > lukesMass) {
      biggerMasses.push(character.name);
    }
  });

  return biggerMasses.join(' - ');
}
...
const sortBy = (property, arr) => {
  console.log(`PROPERTY: ${property} || ARR: ${arr}`);
  return arr.sort((a, b) => {
